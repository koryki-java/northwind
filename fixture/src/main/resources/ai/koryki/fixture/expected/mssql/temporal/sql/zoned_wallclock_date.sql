...
SELECT
  CAST(((CAST(c.date_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS DATE) AS date_new_york
, DATEADD(MONTH, 3, CAST(((CAST(c.date_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS DATE)) AS date_new_york_plus1q
, DATEADD(HOUR, 1, CAST(CAST(((CAST(c.date_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS DATE) AS DATETIME2)) AS date_new_york_plus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2