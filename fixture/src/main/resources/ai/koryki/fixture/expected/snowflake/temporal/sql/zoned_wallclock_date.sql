...
SELECT
  CAST(CONVERT_TIMEZONE('America/New_York', 'UTC', CAST(c.date_new_york AS TIMESTAMP_NTZ)) AS DATE) AS date_new_york
, CAST(CONVERT_TIMEZONE('America/New_York', 'UTC', CAST(c.date_new_york AS TIMESTAMP_NTZ)) AS DATE) + INTERVAL '3 month' AS date_new_york_plus1q
, CAST(CONVERT_TIMEZONE('America/New_York', 'UTC', CAST(c.date_new_york AS TIMESTAMP_NTZ)) AS DATE) + INTERVAL '1 hour' AS date_new_york_plus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2