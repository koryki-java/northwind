...
SELECT
  CASE WHEN CAST(strftime('%Y', CURRENT_DATE) AS INTEGER) >= 2026 THEN 'yes' ELSE 'no' END AS today_is_current
, CASE WHEN CAST(strftime('%Y', CURRENT_TIMESTAMP) AS INTEGER) = CAST(strftime('%Y', CURRENT_DATE) AS INTEGER) THEN 'yes' ELSE 'no' END AS now_agrees_with_today
, CASE WHEN CAST(strftime('%Y', CURRENT_TIMESTAMP) AS INTEGER) = CAST(strftime('%Y', CURRENT_DATE) AS INTEGER) THEN 'yes' ELSE 'no' END AS clock_now_agrees_with_today
FROM
 orders o
WHERE
  o.order_id = 10248