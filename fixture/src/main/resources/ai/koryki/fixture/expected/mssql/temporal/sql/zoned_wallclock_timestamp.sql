...
SELECT
  CAST(((CAST(c.ts_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS datetime2) AS ts_new_york
, DATEADD(HOUR, 1, CAST(((CAST(c.ts_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS datetime2)) AS ts_new_york_plus1h
, DATEADD(HOUR, -1, CAST(((CAST(c.ts_new_york AS datetime2) AT TIME ZONE 'Eastern Standard Time') AT TIME ZONE 'UTC') AS datetime2)) AS ts_new_york_minus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2