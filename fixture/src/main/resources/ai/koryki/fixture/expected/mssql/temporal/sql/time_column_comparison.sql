...
FROM
 check_temporal c
WHERE
  CAST(FLOOR(c.time_from_integer / 10000.0) AS INT) * 3600 + (CAST(FLOOR(c.time_from_integer / 100.0) AS INT) % 100) * 60 + (c.time_from_integer % 100) = DATEDIFF(SECOND, '00:00:00', CAST(c.time_from_string AS TIME))