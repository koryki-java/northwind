...
SELECT
  CAST(CONVERT_TZ(CAST(c.date_new_york AS DATETIME), 'America/New_York', 'UTC') AS DATE) AS date_new_york
, CAST(CONVERT_TZ(CAST(c.date_new_york AS DATETIME), 'America/New_York', 'UTC') AS DATE) + INTERVAL '3' MONTH AS date_new_york_plus1q
, CAST(CONVERT_TZ(CAST(c.date_new_york AS DATETIME), 'America/New_York', 'UTC') AS DATE) + INTERVAL '1' HOUR AS date_new_york_plus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2