...
SELECT
  CAST(with_timezone(c.ts_new_york, 'America/New_York') AT TIME ZONE 'UTC' AS TIMESTAMP) AS ts_new_york
, CAST(with_timezone(c.ts_new_york, 'America/New_York') AT TIME ZONE 'UTC' AS TIMESTAMP) + INTERVAL '1' HOUR AS ts_new_york_plus1h
, CAST(with_timezone(c.ts_new_york, 'America/New_York') AT TIME ZONE 'UTC' AS TIMESTAMP) - INTERVAL '1' HOUR AS ts_new_york_minus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2