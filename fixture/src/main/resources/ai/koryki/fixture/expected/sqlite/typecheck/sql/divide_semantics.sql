...
SELECT
  CAST(t.type_integer AS REAL) / NULLIF(t.type_integer, 0) AS int_div_same
, CAST(7 AS REAL) / 2 AS int_div
, CAST(t.type_decimal AS REAL) / NULLIF(t.type_smallint, 0) AS dec_div
, CAST(t.type_integer AS REAL) / NULLIF((t.type_integer - t.type_integer), 0) AS by_zero
FROM
 check_type t