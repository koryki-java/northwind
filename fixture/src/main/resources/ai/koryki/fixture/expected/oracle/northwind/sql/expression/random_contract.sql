...
SELECT
  CASE WHEN COUNT(DISTINCT DBMS_RANDOM.VALUE) > 1 THEN 'yes' ELSE 'no' END AS varies_per_row
, CASE WHEN min(DBMS_RANDOM.VALUE) >= 0 THEN 'yes' ELSE 'no' END AS never_negative
, CASE WHEN max(DBMS_RANDOM.VALUE) < 1 THEN 'yes' ELSE 'no' END AS never_reaches_one
FROM
 orders o