...
SELECT
  CASE WHEN COUNT(DISTINCT UNIFORM(0::float, 1::float, RANDOM())) > 1 THEN 'yes' ELSE 'no' END AS varies_per_row
, CASE WHEN min(UNIFORM(0::float, 1::float, RANDOM())) >= 0 THEN 'yes' ELSE 'no' END AS never_negative
, CASE WHEN max(UNIFORM(0::float, 1::float, RANDOM())) < 1 THEN 'yes' ELSE 'no' END AS never_reaches_one
FROM
 orders o