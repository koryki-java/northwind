...
FROM
 check_temporal c
WHERE
  CAST(FLOOR(c.time_from_integer / 10000.0) AS INTEGER) * 3600 + MOD(CAST(FLOOR(c.time_from_integer / 100.0) AS INTEGER), 100) * 60 + MOD(c.time_from_integer, 100) = HOUR(CAST(c.time_from_string AS TIME)) * 3600 + MINUTE(CAST(c.time_from_string AS TIME)) * 60 + SECOND(CAST(c.time_from_string AS TIME))