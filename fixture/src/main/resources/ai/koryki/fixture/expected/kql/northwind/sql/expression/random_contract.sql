...
SELECT
  CASE WHEN COUNT(DISTINCT random()) > 1 THEN 'yes' ELSE 'no' END AS varies_per_row
, CASE WHEN min(random()) >= 0 THEN 'yes' ELSE 'no' END AS never_negative
, CASE WHEN max(random()) < 1 THEN 'yes' ELSE 'no' END AS never_reaches_one
FROM
 orders o