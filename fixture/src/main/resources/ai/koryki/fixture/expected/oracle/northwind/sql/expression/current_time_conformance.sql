...
, CASE WHEN EXTRACT(YEAR FROM CURRENT_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) THEN 'yes' ELSE 'no' END AS now_agrees_with_today
, CASE WHEN EXTRACT(YEAR FROM CURRENT_TIMESTAMP) = EXTRACT(YEAR FROM CURRENT_DATE) THEN 'yes' ELSE 'no' END AS clock_now_agrees_with_today
FROM
 orders o
WHERE
  o.order_id = 10248