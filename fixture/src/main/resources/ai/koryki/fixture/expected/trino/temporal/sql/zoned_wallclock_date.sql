...
SELECT
  CAST(with_timezone(CAST(c.date_new_york AS TIMESTAMP), 'America/New_York') AT TIME ZONE 'UTC' AS DATE) AS date_new_york
, CAST(with_timezone(CAST(c.date_new_york AS TIMESTAMP), 'America/New_York') AT TIME ZONE 'UTC' AS DATE) + INTERVAL '3' MONTH AS date_new_york_plus1q
, CAST(CAST(with_timezone(CAST(c.date_new_york AS TIMESTAMP), 'America/New_York') AT TIME ZONE 'UTC' AS DATE) AS TIMESTAMP) + INTERVAL '1' HOUR AS date_new_york_plus1h
FROM
 check_temporal c
WHERE
  c.nr <= 2