...
SELECT
  CAST(ROUND(1.7, 0) AS INT) AS i_pos
, CAST(ROUND(-1.7, 0) AS INT) AS i_neg
, CAST(ROUND(2.5, 0) AS INT) AS i_half
, CAST(ROUND(1.7, 0) AS BIGINT) AS b_pos
, CAST(ROUND(1.7, 0) AS SMALLINT) AS s_pos
FROM
 products p
WHERE
  p.product_id = 1