...
SELECT
  CASE WHEN COUNT(DISTINCT RAND(CHECKSUM(NEWID()))) > 1 THEN 'yes' ELSE 'no' END AS varies_per_row
, CASE WHEN min(RAND(CHECKSUM(NEWID()))) >= 0 THEN 'yes' ELSE 'no' END AS never_negative
, CASE WHEN max(RAND(CHECKSUM(NEWID()))) < 1 THEN 'yes' ELSE 'no' END AS never_reaches_one
FROM
 orders o