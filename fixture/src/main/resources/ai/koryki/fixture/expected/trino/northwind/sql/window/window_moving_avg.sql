...
, p.unit_price
, p.product_name
, avg(CAST(p.unit_price AS DOUBLE)) OVER (PARTITION BY c.category_name ORDER BY p.unit_price, p.product_name ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS moving_avg_price
FROM
 products p
