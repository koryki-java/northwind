...
SELECT
  CASE WHEN year(CAST(GETDATE() AS DATE)) >= 2026 THEN 'yes' ELSE 'no' END AS today_is_current
, CASE WHEN year(CURRENT_TIMESTAMP) = year(CAST(GETDATE() AS DATE)) THEN 'yes' ELSE 'no' END AS now_agrees_with_today
, CASE WHEN year(CURRENT_TIMESTAMP) = year(CAST(GETDATE() AS DATE)) THEN 'yes' ELSE 'no' END AS clock_now_agrees_with_today
FROM
 orders o
WHERE
  o.order_id = 10248